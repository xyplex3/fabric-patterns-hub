...
# Filter script for grafana-dashboard-audit pattern
...
# Remove triple backticks from code blocks if they appear alone on a line (keep JSON blocks)
# But preserve properly formatted code blocks
content=$(echo "$content" | awk '
/^```json$/ { in_json = 1; print; next }
/^```$/ && in_json { in_json = 0; print; next }
/^```[a-z]*$/ && !in_json { next }
/^```$/ && !in_json { next }
{ print }
')
