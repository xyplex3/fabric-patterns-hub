...
# Filter script for python-security-audit pattern
...
content=$(echo "$content" | sed -E '/^\[No (CRITICAL|HIGH|MEDIUM|LOW|INFO) findings\]$/d')
...
/^## (CRITICAL|HIGH|MEDIUM|LOW|INFO)/ {
    ...
    while (length($0) == 0 || $0 ~ /^---$/) {
        if ($0 ~ /^---$/) {
            ...
            getline
            if ($0 ~ /^## (CRITICAL|HIGH|MEDIUM|LOW|INFO|Security|Quick Wins|Additional Resources|Summary)/) {
...
# Preserve code blocks properly - don't strip Python code blocks
content=$(echo "$content" | awk '
/^```python$/ { in_code = 1; print; next }
